...
p.windowed = 0 ; % 1 = small,    窗口大小 0是全屏，1是小屏幕
p.port =0;
winSize = [2000,1000];
winWidth = 34.0; %cm
viewDist = 57.0; %cm
widthDeg = winWidth/viewDist/pi*180;
deg = winSize(1)/widthDeg;
% 
...
% p.stimEcc = deg2pix(5,p.vDist,p.px);                   % 196 eccentricity of sample stimuli
...
fNameall_training = [pwd,'\Subject Data\subID-',p.subID,'-',p.subName(1:end-1),'-load',num2str(p.load),'.mat'] ;

if exist( fNameall_training ,'file')
    load (fNameall_training,'stim'); 
    stim1 = stim;
    clear stim
    stim.tgOriraw = stim1.tgOriraw;
    stim.tg2Oriraw = stim1.tg2Oriraw;
elseif ~isempty(str2num(p.subName(end)))
    error('Please check the information of subject.');
    Screen('CloseAll'); 
else
    if p.group==1
...
for b = 1:p.nBlocks
    ...
    %-------------------------------------------------------
        ...
        text1 = [num2str(p.nBlocks-(b-1))];   %内容
        tCenter1 = [p.xCenter-152,p.yCenter-64];   %位置【x,y】
        Screen('TextSize', w, [55]);
        Screen('DrawText', w, text1, tCenter1(1), tCenter1(2), p.txtCol);   %呈现    
